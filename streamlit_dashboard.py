...
class PerformanceDashboard:
    ...
    def create_performance_evolution_chart(self):
        ...
        # Create DataFrame
        df = pd.DataFrame({"Method": methods, "RPS": rps_values, "Log_RPS": np.log10(rps_values)})
    ...
    def create_latency_comparison_chart(self):
        ...
        df = pd.DataFrame({"Method": methods, "Latency_ns": latencies})
    ...
    def create_scalability_analysis(self):
        ...
        if not scaling_df.empty:
            ...
            fig.update_layout(
                xaxis_title="Task Count", yaxis_title="Tasks per Second (RPS)", height=500
            )
    ...
    def create_system_metrics_dashboard(self):
        ...
        with col1:
            st.metric(label="🖥️ CPU Cores", value=f"{system_specs['cpu_cores']}")

        with col2:
            st.metric(
                label="⚡ CPU Frequency", value=f"{system_specs['cpu_frequency_mhz']:.0f} MHz"
            )

        with col3:
            st.metric(label="💾 Memory", value=f"{system_specs['total_memory_gb']:.1f} GB")

        with col4:
            if "max_threads" in system_specs:
                st.metric(label="🧵 Max Threads", value=f"{system_specs['max_threads']}")
    ...
    def run_dashboard(self):
        ...
        st.set_page_config(
            page_title="H_MODEL_Z Performance Dashboard", page_icon="🚀", layout="wide"
        )
        ...
        if selected_section == "🏠 Overview":
            ...
            st.info(
                "💡 Navigate through different sections using the sidebar to explore detailed performance analysis"
            )
            ...
            st.info(
                "💡 Lower latency values indicate faster per-task processing. Note the logarithmic scale."
            )
            ...
            st.info(
                "💡 The radar chart compares normalized performance metrics across different optimization methods."
            )
