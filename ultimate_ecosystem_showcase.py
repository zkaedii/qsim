...
def display_flash_loan_analysis():
    ...
    if check_file_exists(report_file, "Flash Loan Ecosystem Report"):
        try:
            ...
            print(
                f"   🏊 Avg Liquidity Impact: {impact.get('average_liquidity_impact', 'N/A'):.6f}"
            )
            print(
                f"   📈 Avg Volatility Impact: {impact.get('average_volatility_impact', 'N/A'):.6f}"
            )
...
def run_complete_demonstration():
    ...
    demos = [
        ...
        (
            "npx hardhat run scripts/trading_ecosystem_demo.js --network localhost",
            "Trading Ecosystem Demo",
        ),
    ]
