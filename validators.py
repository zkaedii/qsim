...
class PerformanceDataSchema(BaseModel):
    ...
    requests_per_second: float = Field(..., gt=0, description="Requests per second (must be positive)")
...
class DataValidator:
    ...
    def validate_performance_data(data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not isinstance(perf_data, dict):
                raise SchemaValidationError(
                    field='performance_evolution',
                    expected='dict',
                    actual=str(type(perf_data))
                )
            ...
            raise SchemaValidationError(
                ...
                expected='valid performance data',
                actual=str(e)
            )
    ...
    def validate_statistical_properties(values: List[float]) -> Dict[str, Any]:
        ...
        if not values:
            raise DataIntegrityError(
                check_name='non_empty_values',
                details="Cannot compute statistics on empty list"
            )
        ...
        if not all(np.isfinite(v) for v in values):
            raise DataIntegrityError(
                ...
                details="Values contain NaN or infinite values"
            )
    ...
    def validate_data_consistency(
        ...
        if not common_keys:
            raise DataIntegrityError(
                check_name='common_keys',
                details="No common keys between data sets"
            )
