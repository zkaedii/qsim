...
class TestDataGenerator:
    ...
    def generate_insufficient_data() -> Dict[str, Any]:
        ...
        return {"performance_evolution": {"single_method": 5000000}}
...
class TestScenarios:
    ...
    def normal_operation() -> Dict[str, MockDataSource]:
        ...
        return {
            ...
            "ultimate_speed": MockDataSource(TestDataGenerator.generate_ultimate_speed_results()),
    ...
    def invalid_data() -> Dict[str, MockDataSource]:
        ...
        return {"performance_comparison": MockDataSource(TestDataGenerator.generate_invalid_data())}

    @staticmethod
    def insufficient_data() -> Dict[str, MockDataSource]:
        """Test scenario: Insufficient data for analysis."""
        return {
            "performance_comparison": MockDataSource(TestDataGenerator.generate_insufficient_data())
        }
...
class PerformanceAssertion:
    ...
    def assert_rps_within_range(
        actual_rps: float, expected_rps: float, tolerance_pct: float = 10.0
    ):
        """
    ...
    def assert_performance_improvement(
        baseline_rps: float, optimized_rps: float, min_improvement_pct: float = 10.0
    ):
        """
    ...
    @staticmethod
    def assert_statistical_significance(p_value: float, alpha: float = 0.05):
        """
        ...
        assert p_value < alpha, (
            f"Result not statistically significant: " f"p={p_value:.4f} >= α={alpha}"
        )
...
def main():
    ...
    scenarios = ["normal_operation", "partial_data", "no_data", "invalid_data", "insufficient_data"]
