...
class H_MODEL_Z_FlashLoanImpactAnalyzer:
    ...
    def A_i(self, i: int, t: float) -> float:
        ...
        flash_impact = 0.0
        recent_loans = [event for event in self.flash_loan_events 
                       if abs(event.timestamp - t) <= 5.0]  # 5 time unit window
    ...
    def D_i(self, i: int) -> float:
        ...
        recent_loan_count = len([e for e in self.flash_loan_events 
                               if abs(e.timestamp - max([e.timestamp for e in self.flash_loan_events] or [0])) <= 10])
    ...
    def H_hat(self, t: float) -> float:
        ...
        try:
            ...
            integral_term, _ = quad(
                lambda x: softplus(self.a * (x - self.x0) ** 2 + self.b) * self.f(x) * self.g_prime(x), 
                0, min(t, 10)  # Limit integration range for performance
            )
            ...
            memory_feedback = self.eta * (H_tau + flash_memory / 1000.0) * sigmoid(self.gamma * H_tau)
            
            # Dynamic noise based on market stress
            stress_multiplier = 1.0 + len([e for e in self.flash_loan_events 
                                         if abs(e.timestamp - t) <= 5.0]) * 0.1
            noise = self.sigma * stress_multiplier * self.normal(0, np.sqrt(1 + self.beta * abs(H_prev)))
    ...
    def simulate_flash_loan(self, t: float, borrower: str, asset: str, amount: float, 
                          strategy: str) -> FlashLoanEvent:
        ...
        logger.info(f"Flash loan simulated: {strategy} {amount:.2f} {asset}, "
    ...
    def simulate_flash_loan_ecosystem(self, T: int = 100, num_flash_loans: int = 20) -> Dict:
        ...
        logger.info(f"Simulating flash loan ecosystem for {T} time units with {num_flash_loans} flash loans")
        ...
        success_rate = successful_loans / len(self.flash_loan_events) if self.flash_loan_events else 0
        ...
        logger.info(f"Simulation complete. Success rate: {success_rate:.2%}, "
                   f"Total volume: {total_flash_loan_volume:.2f}, "
                   f"Total profit: {total_arbitrage_profit:.4f}")
        
        return results

    def visualize_flash_loan_impact(self, T: int = 100, output_file: str = "flash_loan_impact.svg") -> Dict:
        ...
        fig.suptitle('H_MODEL_Z Flash Loan Ecosystem Impact Analysis', fontsize=16, fontweight='bold')
        ...
        if self.flash_loan_events:
            ...
            ax2.scatter(successful_times, successful_amounts, c='green', alpha=0.7, 
                       label='Successful', s=50)
            ax2.scatter(failed_times, failed_amounts, c='red', alpha=0.7, 
                       label='Failed', s=50)
        ...
        if self.flash_loan_events:
            ...
            success_rates = [strategies[s]["success"] / strategies[s]["count"] for s in strategy_names]
            ...
            bars1 = ax4.bar(x_pos - 0.2, success_rates, 0.4, label='Success Rate', color='lightblue')
            bars2 = ax4_twin.bar(x_pos + 0.2, avg_profits, 0.4, label='Avg Profit', color='lightcoral')
    ...
    def generate_flash_loan_report(self, output_file: str = "flash_loan_report.json") -> None:
        ...
        for strategy, stats in strategy_stats.items():
            ...
            stats["profit_margin"] = stats["total_profit"] / stats["total_volume"] if stats["total_volume"] > 0 else 0
        ...
        report = {
            ...
            "market_impact_summary": {
                ...
                "average_liquidity_impact": np.mean([imp.liquidity_impact for imp in self.market_impacts]),
                "average_volatility_impact": np.mean([imp.volatility_impact for imp in self.market_impacts]),
...
def main():
    ...
    results = analyzer.visualize_flash_loan_impact(T=100, output_file="h_model_z_flash_loan_impact.svg")
