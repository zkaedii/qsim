    ...
    try:
        ...
        try:
            integral_term, _ = quad(
                ...
                0,
                min(t, 10.0),  # Limit integration range
        ...
        H_t = np.clip(
            sum_term + integral_term + drift + memory_feedback + noise + control, -1000, 1000
        )
        H_hist[t] = H_t
        return H_t
...
def simulate_flash_loan_impact(T=50, output_file="h_model_z_advanced_impact.svg"):
    ...
    try:
        ...
        ax.plot(
            range(T), values, marker="o", linestyle="-", color="gold", linewidth=2, markersize=4
        )
        ax.set_title(
            "🏆 H_MODEL_Z Advanced Mathematical Model H_hat(t)",
            fontsize=14,
            color="gold",
            fontweight="bold",
        )
