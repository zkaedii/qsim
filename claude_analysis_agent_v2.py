...
class ConfigurationManager:
    ...
    def _validate_config(self):
        ...
        for section in required_sections:
            if section not in self.config:
                raise InvalidConfigurationError(
                    config_key=section,
                    reason=f"Required section '{section}' missing from config"
                )
    ...
    def get_required(self, key: str) -> Any:
        ...
        if value is None:
            raise InvalidConfigurationError(
                config_key=key,
                reason=f"Required configuration key '{key}' not found"
            )
        return value
    ...
    def load(self) -> Dict[str, Any]:
        ...
        if not self.filepath.exists():
            if self.required:
                ...
                self.logger.warning(
                    "Optional data source not found",
                    filepath=str(self.filepath)
                )
                return {}
        
        # File exists but couldn't be loaded
        raise DataSourceCorruptedError(
            str(self.filepath),
            str(last_error)
        )
...
class StatisticalAnalyzer:
    ...
    def detect_outliers(
        ...
        threshold = threshold_std_dev or self.config.get(
            'statistics.outlier_threshold_std_dev',
            2.0
        )
...
class ClaudeAnalysisAgentV2:
    ...
    def __init__(
        ...
        self.audit = AuditTrail(
            self.config.get('audit.log_file', 'audit_trail.jsonl')
        )
    ...
    def _get_performance_threshold(self, threshold_name: str) -> float:
        ...
        return self.config.get(
            f'performance_thresholds.{threshold_name}',
            0
        )
    ...
    def _analyze_performance_evolution(
        self,
        ...
        return {
            ...
            'performance_gap': sorted_methods[0][1] / sorted_methods[-1][1] if len(sorted_methods) > 1 else 1.0,
    ...
    def _identify_bottlenecks_mechanistic(
        ...
        for method, rps in perf_data.items():
            ...
            elif any(pattern in method_lower for pattern in self.config.get('bottleneck_classification.jit_patterns', [])):
                # JIT compiled methods
                bottleneck_type = "JIT-compiled: CPU bound with optimization potential"
                category = 'cpu_bound_methods'
            elif any(pattern in method_lower for pattern in self.config.get('bottleneck_classification.async_patterns', [])):
            ...
            elif any(pattern in method_lower for pattern in self.config.get('bottleneck_classification.process_patterns', [])):
    ...
    def _analyze_scaling_patterns(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i in range(1, len(scaling_data)):
            ...
            scaling_insights['scaling_efficiency'][f'{prev_tasks}_{curr_tasks}'] = ScalingEfficiency(
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        self.audit.log_event(
            ...
            description="Comprehensive analysis initiated"
        )
        ...
        try:
            ...
            self.audit.log_event(
                ...
                metadata={'confidence': insights['claude_analysis_summary']['optimization_confidence']}
            )
    ...
    def _save_results(self, analysis_result: Dict[str, Any]):
        ...
        output_path = self.config.get(
            ...
            'claude_comprehensive_analysis_v2.json'
        )
